...
char *ClientConnect( int clientNum, qboolean firstTime, qboolean isBot ) {
	...
	if ( g_delagHitscan.integer ) {
		trap_SendServerCommand( clientNum, "print \"This server is Unlagged: Server side lag compensation is ON!\n\"" );
	}
	else {
		trap_SendServerCommand( clientNum, "print \"This server is Unlagged: Server side lag compensation is OFF!\n\"" );
	}
	//unlagged - backward reconciliation #5
